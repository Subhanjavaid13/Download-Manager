...
async def create_job(
    ...
    if parsed.kind == "playlist" or not parsed.video_id:
        raise HTTPException(
            400,
            "That is a playlist link. Send it to POST /api/v1/playlists to download "
            "every video, or paste the link to one video instead.",
        )

    ip = client_ip(request, settings)
    enforce_not_banned(bans, user, ip)
    enforce_quota(user, owner, accounts, settings)
