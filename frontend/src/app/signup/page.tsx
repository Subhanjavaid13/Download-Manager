      ...
      <Page>
        ...
        <Card>
          ...
          <p className="mt-2 text-sm text-ink-2">
            We sent a verification link to <strong>{sentTo}</strong>. Tap it on this device to finish
            signing up. Downloads unlock as soon as your email is verified.
          </p>
          <p className="mt-2 text-xs text-muted">
            Nothing there after a minute? Check spam, or resend below. The free email service allows
            only a few messages per hour.
          </p>
          {error && <div className="mt-3"><Notice tone="error">{error}</Notice></div>}
          {resent && <div className="mt-3"><Notice tone="ok">Sent again.</Notice></div>}
          <div className="mt-4 flex gap-3">
            <Button type="button" tone="secondary" onClick={resend} disabled={resent}>
              Resend email
            </Button>
            <Link href="/signin" className="self-center text-sm font-medium text-accent hover:underline">
              Go to sign in
            </Link>
          </div>
        </Card>
    ...
    <Page>
      ...
      <Card>
        ...
        <p className="mt-1 text-sm text-muted">
          {config ? `${config.anon_daily_limit} downloads a day as a guest, 20 with an account.` : ""}
        </p>
        {signupOff && (
          <div className="mt-3">
